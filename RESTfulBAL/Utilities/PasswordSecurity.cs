...
namespace RESTfulBAL.Utilities.PasswordSecurity
{
    ...
    class PasswordStorage
    {
        ...
        public void CreateHash(string password)
        {
            ...
            // oldformat: algorithm:iterations:hashSize:salt:hash
            // newformat: algorithm:iterations:hashSize:hash
            String parts = "sha1:" +
                    PBKDF2_ITERATIONS +
                    ":" +
                    hash.Length +
        ...
        public bool VerifyPassword(string password)
        {
            ...
            try
            {
                salt = Convert.FromBase64String(Saltstring.ToString());//split[SALT_INDEX]);
            }
            catch (ArgumentNullException ex)
            {
                throw new CannotPerformOperationException(
        ...
        public bool ComparePasswordHash(string FullpasswordHashString, string ParsedpasswordHashString)
        {
            char[] delimiter = { ':' };
            string[] split = FullpasswordHashString.Split(delimiter);
            ...
            {
                throw new CannotPerformOperationException(
                    "Hash1: Invalid argument given to Convert.FromBase64String",
                    ex
                );
            }
            catch (FormatException ex)
            ...
            try
            {
                hash2 = Convert.FromBase64String(ParsedpasswordHashString);
            }
            catch (ArgumentNullException ex)
            {
                throw new CannotPerformOperationException(
                    "Hash2: Invalid argument given to Convert.FromBase64String",
                    ex
                );
            }
            catch (FormatException ex)
